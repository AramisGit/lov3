# lov3
A little 3D implmentation of the love2D engine using shadow volumes

This is just a hobby project, mostly to test the feasibility of shadow volumes. Anyone is free to do what they like with this.

This requires love2D, and the entirety of the "3d-ification" of the egnine lives in the single lov3.lua file.

Below is a simple example scene with some point lights (all lights are point lights in this implementation)

![alt text](example.png "with shadows") ![alt text](example_with_volumes.png "with debug volumes")
